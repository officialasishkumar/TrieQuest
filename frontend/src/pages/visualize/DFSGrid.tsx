...
export default function DFSGridViz() {
  ...
  const resetAll = useCallback(() => { setGrid(initGrid()); setPathLength(null); cancelRef.current = true; }, []);
  ...
  const startDFS = useCallback(async () => {
    ...
    async function dfs(r: number, c: number): Promise<boolean> {
      if (cancelRef.current) return false;
      const key = `${r},${c}`;
      if (r < 0 || r >= ROWS || c < 0 || c >= COLS) return false;
      if (visited.has(key) || g[r][c] === "wall") return false;
      visited.add(key);

      if (r === tgt![0] && c === tgt![1]) return true;
  ...
  const cellColor = (type: CellType) => {
    switch (type) {
      ...
      case "visited": return "bg-violet-400/50 dark:bg-violet-500/40";
    ...
    <div className="flex flex-1 flex-col bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/5 via-background to-secondary/20 w-full">
      <div className="w-full max-w-6xl mx-auto px-6 pt-6 pb-16 space-y-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">DFS Pathfinder</h1>
            <p className="text-sm text-muted-foreground">Explore a grid using depth-first search.</p>
          </div>
        </motion.div>
        <div className="flex gap-4 text-xs">
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-violet-400/50 inline-block" /> Visited</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-amber-400 inline-block" /> Path</span>
          {pathLength !== null && <span className="ml-auto font-bold">Path Length: {pathLength}</span>}
        </div>
        ...
        >
          <div className="inline-grid gap-px" style={{ gridTemplateColumns: `repeat(${COLS}, ${CELL_SIZE}px)` }}>
            {grid.map((row, r) =>
              row.map((cell, c) => (
                <div key={`${r}-${c}`} className={`transition-colors duration-150 rounded-sm border border-border/30 cursor-pointer ${cellColor(cell)}`} style={{ width: CELL_SIZE, height: CELL_SIZE }}
                  onMouseDown={() => toggleWall(r, c)} onMouseEnter={() => { if (draggingRef.current) toggleWall(r, c); }}>
        ...
        <div className="p-4 rounded-2xl bg-card border shadow-sm space-y-4">
          <div className="flex items-center gap-4">
            ...
            <input type="range" min={5} max={200} step={5} value={speed} onChange={e => setSpeed(Number(e.target.value))} className="flex-1 accent-primary" />
          </div>
          <div className="flex flex-wrap gap-2">
            <Button onClick={startDFS} variant={running ? "destructive" : "default"} className="gap-2">
              {running ? <><Square className="w-4 h-4" /> Stop</> : <><Play className="w-4 h-4" /> Start DFS</>}
            </Button>
          ...
          <p className="text-xs text-muted-foreground">Click or drag to place walls. DFS explores depth-first — note the path may not be shortest.</p>
        </div>
      </div>
    </div>
  );
